...
//
//   node render.mjs                          # both cuts -> out/, then encodes
//   node render.mjs --mode square            # one cut
//   node render.mjs --stills 9.5,16.8        # PNG stills only (visual QA)
...
async function renderChunk(browser, port, mode, fps, from, to, file) {
...
async function main() {
  ...
  try {
    for (const mode of modes) {
      const probe = await openPage(browser, port, mode);
      ...
      if (stills.length) {
        ...
        for (const t of stills) {
          const file = path.join(outDir, "stills", `${mode}-${t.toFixed(2)}.png`);
      ...
      const inter = path.join(outDir, `intermediate-${mode}.mp4`);
      if (!flag("encode-only")) {
        ...
        const chunks = Array.from({ length: workers }, (_, i) => ({
          ...
          file: path.join(outDir, `chunk-${mode}-${i}.mp4`),
        })).filter((c) => c.from < c.to);
        await Promise.all(
          chunks.map((c) => renderChunk(browser, port, mode, fps, c.from, c.to, c.file)),
        );
        const list = path.join(outDir, `chunks-${mode}.txt`);
        await writeFile(list, chunks.map((c) => `file '${c.file}'`).join("\n"));
        await run("ffmpeg", ["-y", "-f", "concat", "-safe", "0", "-i", list, "-c", "copy", inter]);
        for (const c of chunks) await rm(c.file);
        await rm(list);
        console.log(`[${mode}] ${total} frames in ${((Date.now() - started) / 1000).toFixed(0)}s`);
      }

      const audio = path.join(root, "audio", "soundtrack.wav");
      const hasAudio = await exists(audio);
      const base = path.join(outDir, `launch-${mode}-${VERSION}`);
      ...
      console.log(
        `[${mode}] wrote ${base}.mp4 / .webm / -poster.jpg${hasAudio ? "" : " (no soundtrack found)"}`,
      );

      if (flag("publish")) {
        await mkdir(publishDir, { recursive: true });
        for (const ext of [".mp4", ".webm", "-poster.jpg"]) {
          await copyFile(`${base}${ext}`, path.join(publishDir, `launch-${mode}-${VERSION}${ext}`));
        }
        console.log(`[${mode}] published to ${publishDir}`);
      }
    }
  } finally {
    await browser.close();
